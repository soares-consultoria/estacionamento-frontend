  ...
  if (auth) {
    try {
      ...
      if (user.role === 'SUPER_ADMIN' || user.role === 'SISTEMA_ADMIN') {
        ...
        if (selectedId && config.url && (config.url.includes('/api/dashboard') || config.url.includes('/api/importacao') || config.url.includes('/api/chat-ai') || config.url.includes('/api/eventos'))) {
